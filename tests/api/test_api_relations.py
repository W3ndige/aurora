...
def test_add_relation(client, monkeypatch):
    ...
    parent_response = client.post("/api/v1/sample/", files={'file': (file_1["filename"], file_1["content"])}).json()
    parent_id = parent_response["id"]

    child_response = client.post("/api/v1/sample/", files={'file': (file_2["filename"], file_2["content"])}).json()
